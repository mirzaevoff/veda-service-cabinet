...
export function DevTaskTypesManager({ onChange }: { onChange?: () => void }) {
  ...
  function openEdit(type: DevTaskType) {
    setFormId(type.id);
    setName(type.name);
    setColor(type.color ?? "");
    setOrder(String(type.order));
    setFormOpen(true);
  }
  ...
  async function saveType() {
    ...
    try {
      ...
      if (formId) await devTasksApi.updateType(formId, body);
      else await devTasksApi.createType(body);
  ...
  async function deleteType(id: string) {
    try {
      await devTasksApi.removeType(id);
      toast.success(t("typeDeleted"));
      load();
      onChange?.();
    } catch (e) {
      if (e instanceof SessionExpiredError) router.replace("/login");
      else if (e instanceof ApiError && e.code === "ER2505") toast.error(t("typeInUse"));
      else toast.error(t("genericError"));
    }
  }
              ...
              >
                <div className="flex min-w-0 flex-col gap-1">
                  <div className="flex flex-wrap items-center gap-1.5">
                    <DevTaskTypeBadge slug={type.slug} types={new Map([[type.slug, type]])} />
                    {type.system && (
        ...
        {formOpen ? (
          <div className="flex flex-col gap-3 rounded-lg border border-border p-3">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="flex flex-col gap-1.5">
                <Label className="text-sm font-medium text-muted-foreground">{t("typeColor")}</Label>
                <Input
                  value={color}
                  placeholder="#A21500"
                  maxLength={9}
              ...
              <div className="flex flex-col gap-1.5">
                <Label className="text-sm font-medium text-muted-foreground">{t("typeOrder")}</Label>
                <Input
                  type="number"
                  min={0}
                  max={999}
